    ...
    QStandardItemModel(rows,columns,parent)
{
    //    ColorDark=new QColor(255,168,111);
    //    ColorDiluted =new QColor(255,204,170);
    ColorDark=new QColor("#b4f4f7");
    ColorDiluted =new QColor("#d9faf7");
    ...
    case Qt::ToolTipRole :{
        if(orientation==Qt::Vertical)
        {
            if (VerticalHeaderlToolTip.isEmpty()) {
                return qVariantFromValue("Delete "+headerData(section,Qt::Vertical).toString()) ;
            } else {
        ...
        if(orientation==Qt::Horizontal)
        {
            if (HorizontalHeaderlToolTip.isEmpty()) {
                ...
                if (morder[section]==Qt::AscendingOrder) {
                    html= "<html><head/><body><p><span><img src=\":/image/botom.png\"/> Descending Order in </span><span style=\" font-weight:600; color:#ff5500;\">"+headerData(section,Qt::Horizontal).toString()+"</span></p></body></html>";


                } else {
                    html= "<html><head/><body><p><span><img src=\":/image/top.png\"/> Ascending Order in </span><span style=\" font-weight:600; color:#ff5500;\">"+headerData(section,Qt::Horizontal).toString()+"</span></p></body></html>";
                }
                return qVariantFromValue(html) ;
            }
        }
